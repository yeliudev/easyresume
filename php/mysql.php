...
define('SQL_SELECT_RESUME', "SELECT name,sex,avatarUrl,birthdate,birthplace,cellphone,email,residence,address,degree,institution,major,awards,work_time,job_status,salary_type,salary,jobs,cpp_ability,py_ability,java_ability,cs_ability,git_ability,latax_ability,statement,last_modify FROM user WHERE username=? LIMIT 1");
define('SQL_UPDATE_ALL', "UPDATE user SET name=?,sex=?,avatarUrl=?,birthdate=FROM_UNIXTIME(?),birthplace=?,cellphone=?,email=?,residence=?,address=?,degree=?,institution=?,major=?,awards=?,work_time=?,job_status=?,salary_type=?,salary=?,jobs=?,cpp_ability=?,py_ability=?,java_ability=?,cs_ability=?,git_ability=?,latax_ability=?,statement=?,last_modify=FROM_UNIXTIME(?) WHERE username=?");
define('SQL_UPDATE', "UPDATE user SET name=?,sex=?,birthdate=FROM_UNIXTIME(?),birthplace=?,cellphone=?,email=?,residence=?,address=?,degree=?,institution=?,major=?,awards=?,work_time=?,job_status=?,salary_type=?,salary=?,jobs=?,cpp_ability=?,py_ability=?,java_ability=?,cs_ability=?,git_ability=?,latax_ability=?,statement=?,last_modify=FROM_UNIXTIME(?) WHERE username=?");
